...
def generate_reading(user_id):
    ...
    if user.profile.readings:
        ...
        obs = env.step(Action(basal=Decimal(latest_reading.basal_injected), bolus=Decimal(latest_reading.bolus_injected)))
    else:
    ...
    # Get noise level
    noise = GlucoseReading.get_noise_level()

    # Get final reading
    adjusted_read = GlucoseReading.adjust_glucose_reading(reading, noise)
