...
class MyController(Controller):
    def __init__(self, init_state, patient_id, env, scenario):
        ...
        # Track CGM noise event timing
        self.last_noise_time = None
        self.noise_duration = 0

    def adjust_for_noise(self, reading):
        """Simulates CGM signal loss intermittently and removes glucose readings based on ISO noise standards."""

        # Get current simulation time in hours
        sim_time = (self.env.time - self.scenario.start_time).total_seconds() / 3600

        # Introduce noise event every 4 hours (50% chance)
        if self.last_noise_time is None or sim_time - self.last_noise_time >= 4:
            if np.random.rand() < 0.5:  # 50% probability of noise event
                self.last_noise_time = sim_time
                self.noise_duration = np.random.uniform(15, 30)  # Noise lasts 15-30 min

        # If currently in a noise period, force reading to 0
        if (
            self.last_noise_time is not None
            and sim_time - self.last_noise_time <= self.noise_duration / 60
        ):
            return 0

        # Apply standard ISO noise filtering
        noise_generator = CGMSensor.withName("Dexcom")._noise_generator
        noise = np.mean(noise_generator.noise) / 18 if noise_generator.noise else 0
    ...
    def policy(self, observation, reward, done, **info):
        ...
        raw_glucose = observation.CGM / 18  # Original reading from sensor
        current_glucose = self.adjust_for_noise(raw_glucose)  # Adjust for noise
...
def run_lossy_connection(patient_id, save_path):
    ...
    controller = MyController(0, patient_id, env, scenario)
